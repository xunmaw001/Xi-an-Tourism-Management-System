    ...
    <div class="addEdit-block">
        <el-form
            ...
            <input id="yonghuId" name="yonghuId" type="hidden">
                <el-col :span="12">
                    <el-form-item class="select" v-if="type!='info'"  label="类型" prop="jingdianCollectionTypes">
                        <el-select v-model="ruleForm.jingdianCollectionTypes" :disabled="ro.jingdianCollectionTypes" placeholder="请选择类型">
                            <el-option
                                v-for="(item,index) in jingdianCollectionTypesOptions"
                    ...
                    <div v-else-if="type=='info'">
                        <el-form-item class="input" label="类型" prop="jingdianCollectionValue">
                        <el-input v-model="ruleForm.jingdianCollectionValue"
...
<script>
    ...
    export default {
        data() {
            return {
                ...
                ro:{
                    jingdianId: false,
                    yonghuId: false,
                    jingdianCollectionTypes: false,
                    insertTime: false,
                },
                ruleForm: {
                    jingdianId: '',
                    yonghuId: '',
                    jingdianCollectionTypes: '',
                    insertTime: '',
                },
                jingdianCollectionTypesOptions : [],
                jingdianOptions : [],
                yonghuOptions : [],
                rules: {
                   jingdianId: [
        ...
        created() {
            ...
            //获取下拉框信息
                this.$http({
                    url:`dictionary/page?page=1&limit=100&sort=&order=&dicCode=jingdian_collection_types`,
                    method: "get"
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.jingdianCollectionTypesOptions = data.data.list;
                    }
                });
        ...
        methods: {
            ...
            info(id) {
                let _this =this;
                _this.$http({
                    url: `jingdianCollection/info/${id}`,
                    method: 'get'
                }).then(({ data }) => {
                    if (data && data.code === 0) {
            ...
            onSubmit() {
                this.$refs["ruleForm"].validate(valid => {
                    if (valid) {
                        this.$http({
                            url:`jingdianCollection/${!this.ruleForm.id ? "save" : "update"}`,
                            method: "post",
                            data: this.ruleForm
                        }).then(({ data }) => {
                            if (data && data.code === 0) {
                                this.$message({
                                    ...
                                    onClose: () => {
                                        ...
                                        this.parent.jingdianCollectionCrossAddOrUpdateFlag = false;
                                        this.parent.search();
